...
public class HouseDetector {
    ...
    public static IntBoundary getClosedField(World world, IntVec3 start) {
        ...
        if (!checkPoint(world, closeList, bound, start, start))
            return null;
        return bound;
    }
    ...
    private static boolean checkPoint(World world, ArrayList<IntVec3> closeList, IntBoundary bound, IntVec3 pos, IntVec3 start) {
        ...
        for (int i = 0; i < 8; i += 2) {
            ...
            if (isEnd(world, dpos)) {
                ...
                if (dpos.x < bound.minx)
                    bound.minx = dpos.x;
                else if (dpos.x > bound.maxx)
                    bound.maxx = dpos.x;

                if (dpos.z < bound.minz)
                    bound.minz = dpos.z;
                else if (dpos.z > bound.maxz)
                    bound.maxz = dpos.z;
            } else {
                if (!checkPoint(world, closeList, bound, dpos, start))
                    return false;
            }
        }
    ...
    private static boolean isOutOfRadius(IntVec3 pos, IntVec3 start) {
        return Math.abs(pos.x - start.x) > SearchRadius || Math.abs(pos.z - start.z) > SearchRadius;
    }
