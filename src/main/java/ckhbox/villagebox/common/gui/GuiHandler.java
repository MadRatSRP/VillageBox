    ...
    public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        ...
        if (ID >= 100 && ID < 200) {
            ...
            if (world.provider.getDimension() == x) {
                ...
                if (villager != null) {
                    switch (ID) {
                        ...
                        case GuiIDs.VillagerTrading:
                            return new ContainerTrading(player.inventory, villager, world);
                        case GuiIDs.VillagerUpgrading:
                            return new ContainerVillagerUpgrading(player.inventory, villager, world);
                        case GuiIDs.VillagerQuest:
            ...
            switch (ID) {
                case GuiIDs.Mail:
                    return new ContainerEmpty();
                case GuiIDs.VillageBook:
                    return new ContainerVillageBook(player);
                default:
                    break;
            }
        }
        ...
        if (ID >= 100 && ID < 200) {
            if (world.provider.getDimension() == x) {
                ...
                if (villager != null) {
                    switch (ID) {
                        case GuiIDs.VillagerMain:
                            return new GuiVillagerMain(player, villager);
                        case GuiIDs.VillagerTrading:
                            return new GuiVillagerTrading(player.inventory, villager, world);
                        case GuiIDs.VillagerUpgrading:
                            return new GuiVillagerUpgrading(player.inventory, villager, world);
                        case GuiIDs.VillagerQuest:
                            return new GuiVillagerQuest(player, villager);
                        default:
                            break;
                    }
                }
            ...
            switch (ID) {
                case GuiIDs.Mail:
                    return new GuiMail(player);
                case GuiIDs.VillageBook:
                    return new GuiVillageBook();
                default:
                    break;
            }
        }
