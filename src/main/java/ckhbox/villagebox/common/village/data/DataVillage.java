...
public class DataVillage extends WorldSavedData {
    ...
    private final ArrayList<HomeBoundary> homeList = new ArrayList<HomeBoundary>();

    private final ArrayList<DeadVillager> deadVillagers = new ArrayList<DeadVillager>();
    ...
    public String addHome(String owner, IntBoundary bound) {
        ...
        for (HomeBoundary homeBoundary : homeList) {
            if (homeBoundary.boundary.contact(bound)) {
                return homeBoundary.owner;
            }
        }
    ...
    public void reviveVillager(DeadVillager deadVillager) {
        ...
        @Nullable MinecraftServer server = this.world.getMinecraftServer();

        if (server != null) {
            server
                    .getPlayerList()
                    .sendChatMsg(new TextComponentTranslation(PathHelper.full("message.villager.revived"),
                            deadVillager.name));
        }
    }
